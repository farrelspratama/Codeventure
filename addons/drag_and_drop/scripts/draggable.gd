...
var a: Area2D = null
var parent_control: Button
var parent_control_container: Control
...
func _ready():
	...
	a = candidate
	if a.get_parent():
		parent_control = a.get_parent()
		if parent_control.get_parent() is not Button:
			parent_control_container = parent_control.get_parent() 
...
func _handle_returning(delta: float) -> void:
	...
	if a.global_position.distance_squared_to(next_position) <= CLOSE_ENOUGH_THRESHOLD:
		...
		if parent_control_container and parent_control.get_parent() != previous_parent: 
			parent_control.reparent(parent_control_container)
...
func _on_input_event(_viewport, event, _shape_idx):
	if event.is_action_pressed(drag_input_name) and state == DRAGGABLE_STATE.IDLE:
		...
		previous_position = a.global_position
		if parent_control:
			previous_parent = parent_control.get_parent()
		else:
		...
		if drag_layer_parent:
			...
			if parent_control:
				parent_control.reparent(drag_layer_parent)
			else:
